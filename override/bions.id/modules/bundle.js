(function () {
    ...
    var BIONS = {
        ...
        makeWire(b) {
            return BIONS.wireSetup(b)
        },
        wireSetup(b) {
            var i, plainUser, k, l = 0, msgHandler = {};
            debug = function (b) {
            ...
            var wireConn = {
                ...
                checkLoginResponse(b) {
                    ...
                    if (b != null)
                        if (b.cmd === BIONS.CMD_CONNECTED && wireConn.onLoggedIn)
                            ...
                            var f = msgHandler[b.bucket[BIONS.txt_subscription]];
                            f && f(b)
                        } else
                ...
                encryptAndSend(cmd, bucket, tail) {
                    cmd = BIONS.encryptData(cmd, bucket, tail);
                    i && i.send(cmd)
                },
                ...
                initWebSocket(user, passcode, onSuccess, onError) {
                    plainUser = user;
                    k = cryptoMD5.en(passcode);
                    wireConn.onLoggedIn = onSuccess;
                    wireConn.onerror = onError;
                    i = new WebSocket(wireConn.ja);
                    i.onmessage = wireConn.checkLoginResponse;
                    i.onclose = function () {
                        var b = BIONS.txt_connection_lost + " " + wireConn.ja;
                        onError && onError(b)
                    }
                        ;
                    i.onopen = function () {
                        var loginReq = {};
                        loginReq[BIONS.txt_login] = plainUser;
                        loginReq[BIONS.txt_passcode] = plainUser == "0M789" ? k : k + "|zaisan";
                        wireConn.encryptAndSend(BIONS.CMD_CONNECT, loginReq)
                    }
                },
                disconnect(b) {
                    wireConn.close(b)
                ...
                close(b) {
                    try {
                        wireConn.encryptAndSend(BIONS.CMD_DISCONNECT),
                            i.onclose = function () { }
                            ,
                            setTimeout(function () {
                                i.close();
                ...
                sendData(b, f, i) {
                    f = f || {};
                    f[BIONS.txt_destination] = b;
                    wireConn.encryptAndSend(BIONS.CMD_SEND, f, i)
                },
                sub(b, d, f) {
                    return wireConn.subscribe(b, d, f)
                },
                subscribe(b, f, i) {
                    var f = f || {}
                        , j = BIONS.txtSubPrefix + l++;
                    return f[BIONS.txt_destination] = b,
                        f[BIONS.ID] = j,
                        msgHandler[j] = i,
                        wireConn.encryptAndSend(BIONS.CMD_SUBSCRIBE, f),
                        j
                },
                unsubscribe(b, d) {
                ...
                sendUnsubscribe(b, f) {
                    ...
                    f[BIONS.ID] = b;
                    delete msgHandler[b];
                    wireConn.encryptAndSend(BIONS.CMD_UNSUBSCRIBE, f)
                },
                getSession() {
    ...
    var ModuleComm = {};
    ModuleComm.wire = BIONS.makeWire;
    window.Comm = ModuleComm;

    var ModuleBundle = {};
    ModuleBundle.en = BIONS.encrypt;
    ModuleBundle.rsa = BIONS.rsa;
    ModuleBundle.enb64 = BIONS.enb64;
    ModuleBundle.deb64 = BIONS.deb64;
